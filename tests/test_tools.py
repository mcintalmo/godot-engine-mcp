...
class MockGodotClient(GodotClient):
    ...
    async def audit_orphan_nodes(
        ...
        return StandardResult(
            success=True,
            message="Orphan node audit: 0 orphan nodes detected (HEALTHY).",
            mode=self.mode,
            data={
                "orphan_node_count": 0,
    ...
    async def capture_profiler_trace(
        ...
        return StandardResult(
            success=True,
            message=f"Captured profiler trace across {frames_to_sample} frames: 16.67 ms/frame (60.0 FPS).",
            mode=self.mode,
            data={
    ...
    async def inspect_vram_usage(
        ...
        return StandardResult(
            success=True,
            message="Inspected GPU VRAM usage: 128.50 MB total (Texture: 85.20 MB, Buffer: 43.30 MB).",
            mode=self.mode,
            data={
    ...
    async def configure_multiplayer_spawner(
        ...
        return StandardResult(
            success=True,
            message=f"Configured MultiplayerSpawner '{node_name}': {', '.join(changes) or 'No modifications'}.",
            mode=self.mode,
            data={
    ...
    async def configure_multiplayer_synchronizer(
        ...
        return StandardResult(
            success=True,
            message=f"Configured MultiplayerSynchronizer '{node_name}': {', '.join(changes) or 'No modifications'}.",
            mode=self.mode,
            data={
    ...
    async def simulate_network_conditions(
        ...
        return StandardResult(
            success=True,
            message=f"Configured simulated network conditions: Latency {latency_ms}ms, Packet Loss {packet_loss_percent:.1f}%, Jitter {jitter_ms}ms, Offline: {offline_mode}.",
            mode=self.mode,
            data={
    ...
    async def configure_lightmap_gi(
        ...
        return StandardResult(
            success=True,
            message=f"Configured {gi_type.upper()} node '{node_name}' under '{parent_path}'.",
            mode=self.mode,
            data={
                "gi_name": node_name,
    ...
    async def bake_lightmaps(
        ...
        return StandardResult(
            success=True,
            message=f"Baked lighting for node '{lightmap_node_path.split('/')[-1]}' (Scope: {bake_mode.upper()}).",
            mode=self.mode,
            data={
    ...
    async def setup_xr_rig(
        ...
        return StandardResult(
            success=True,
            message=f"Scaffolded XROrigin3D rig '{rig_name}' with {len(child_nodes)} child tracking nodes under '{parent_path}'.",
            mode=self.mode,
            data={
                "rig_name": rig_name,
    ...
    async def configure_xr_passthrough(
        ...
        return StandardResult(
            success=True,
            message=f"Configured OpenXR spatial settings (Passthrough: {enable_passthrough}, RefSpace: {reference_space.upper()}, Foveation: {foveated_rendering_level.upper()}).",
            mode=self.mode,
            data={
    ...
    async def dispatch_compute_shader(
        ...
        return StandardResult(
            success=True,
            message=f"Successfully dispatched compute shader with {total_wg} workgroups.",
            mode=self.mode,
            data={
                "workgroup_size": wg,
    ...
    async def inspect_rendering_device(
        ...
        return StandardResult(
            success=True,
            message="Inspected RenderingDevice 'Apple M-Series GPU' (Apple Metal Driver).",
            mode=self.mode,
            data={
                "device_name": "Apple M-Series GPU",
                "vendor_name": "Apple",
                "driver_name": "Metal / Vulkan",
    ...
    async def scatter_multimesh(
        ...
        return StandardResult(
            success=True,
            message=f"Scattered {instance_count} GPU MultiMesh instances across area [{area[0]}m x {area[1]}m] under '{parent_path}'.",
            mode=self.mode,
            data={
                "node_name": node_name,
                "node_path": f"{parent_path}/{node_name}"
    ...
    async def configure_lod_manager(
        ...
        return StandardResult(
            success=True,
            message=f"Configured LOD visibility range [{visibility_range_begin}m to {visibility_range_end}m] for '{node_path.split('/')[-1]}'.",
            mode=self.mode,
            data={
                "node_name": node_path.split("/")[-1],
                "node_path": node_path,
